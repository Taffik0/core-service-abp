...
@router.get("/classes/{class_id}/students", response_model=None)
...
@router.get("/classes/{class_id}/teachers", response_model=None)
async def get_teacher_of_class(
        class_id: int,
        token: AuthToken = Depends(require_auth),
        class_member_serv: ClassMemberService = Depends(get_class_member_service)):
    refs = await class_member_serv.get_teachers_of_class(token.uuid, token.user_type, class_id)
    logger.info(refs)
    return refs
...
@router.delete("/classes/{class_id}/members/{member_uuid}")
async def delete_member(
        class_id: int, member_uuid: UUID,
        token: AuthToken = Depends(require_auth),
        class_member_serv: ClassMemberService = Depends(get_class_member_service)):
    return await class_member_serv.kick_member(token.uuid, token.user_type, member_uuid, class_id)
